     ...
     1    lambda*trace_strainInc + 2.d0*mu*strainInc(1:ndir)

          ! Calculate the shear components
          stressNew(ndir+1:ndir+nshr) = stressOld(ndir+1:ndir+nshr) +
     1    2.d0*mu*strainInc(ndir+1:ndir+nshr)
     ...
     2    equiv_stress_jc  
          ...
          if (eps_rate <= 1D-12) then
              eps_rate = epsilon_dot_zero
          end if

          if (eps_iter <= 1D-12) then
              eps_iter = 0.d0
          end if
     ...
     3    stress_tensor(ndir+nshr), hyd, equiv_stress
          ...
          if (equiv_stress == 0.d0) then
              pressure_stress_ratio = 0.d0
          else
